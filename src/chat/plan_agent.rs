...
impl PlanAgent {
    ...
    pub async fn start(self: Arc<Self>) {
        let s = self.clone();
        if s.process.read().await.is_some() {
            return
        }
        let mut p = s.process.write().await;
    ...
    async fn loop_worker(self: Arc<Self>) -> Result<(), Box<dyn Error>> {
    ...
    async fn on_chat(&mut self, msg: Arc<ChatMessage>) -> Result<(), Box<dyn Error>> {
        ...
        let prompt = Self::get_prompt(&self.profiles_summarize, &self.recent_chats);
    ...
    async fn complete_chat(&self, profile: &Profile) -> Result<(), Box<dyn Error>> {
        ...
        let conversation = self.recent_chats.iter()
